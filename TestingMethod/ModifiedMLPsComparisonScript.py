...
class KlnGrd_Net(torch.nn.Module):
    ...
    def __init__(self, 
        ...
        self.mduls[f"HidnLay{0}"] = torch.nn.Linear(in_features=in_dim, out_features=nodes_per_lyr, bias=True)
...
class PeriodicBC_Network(torch.nn.Module):
    ...
    def __init__(self, 
        ...
        self.mduls[f"HidnLay{0}"] = torch.nn.Linear(in_features=in_dim+1, out_features=nodes_per_lyr, bias=True)
        torch.nn.init.xavier_normal_(self.mduls[f"HidnLay{0}"].weight.data, gain=1.41)
        self.mduls['ActFunc0'] = torch.nn.Tanh()
    ...
    def forward(self, x:torch.Tensor):
        ...
        # y = torch.cat((torch.cos(torch.matmul(embedded, self.kernel)),torch.sin(torch.matmul(embedded, self.kernel)) ), dim=1)
...
def main(dflag:int, ver:int, data_slct_type:str, jobId:int, arrayId:int, node:str):
    ...
    saveName = 'ModMlpsResultsForSlurmJob'+str(jobID)+'ArrayNum'+str(jobVer)
    ...
    os.makedirs(name="ModifiedMlpsRunStuff", exist_ok=True)
    os.chdir('ModifiedMlpsRunStuff')
    ...
    lrnr.WriteResults(data_set_name=data_set, file_name=data_set+"ModMlps", precision=5, true_eq=crctEQ, errors=[err, errs], act_func='Tanh()', **res_dict)
