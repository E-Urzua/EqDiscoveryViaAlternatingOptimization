...
class KlnGrd_Net(torch.nn.Module):
    ...
    def forward(self, x:torch.Tensor):
        ...
        return self.ntwrk(y)*(x[:,0:1].pow(2) - 1)
...
def main(dflag:int, ver:int, data_slct_type:str, jobId:int, arrayId:int, node:str):
    ...
    saveName = 'FourEmbedResultsForSlurmJob'+str(jobID)+'ArrayNum'+str(jobVer)
    ...
    os.makedirs(name="FourierFeatEmbeddRunStuff", exist_ok=True)
    os.chdir('FourierFeatEmbeddRunStuff')
    ...
    lrnr.WriteResults(data_set_name=data_set, file_name=data_set+"FourEmbd", precision=5, true_eq=crctEQ, errors=[err, errs], act_func='Tanh()', **res_dict)
