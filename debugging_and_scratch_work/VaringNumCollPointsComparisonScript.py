...
def main(dflag:int, ver:int, data_slct_type:str, nCpnts:int, jobId:int, arrayId:int, node:str):
    
    jobID , jobVer = jobId, arrayId
    dvc = torch.device('cuda:0') if torch.cuda.is_available() else torch.device('cpu')
    # dvc = torch.device('cpu')

    # (Dname, nDpnts, nTrn, _, noise, data_seed, sptl_ord, tmp_order, polyDeg, 
    ...
    saveName = f"VaryingNumCollPointsResultsForSlurmJob"+str(jobID)+'ArrayNum'+str(jobVer)
    ...
    os.makedirs(name="VaryingNumCollPointsRunStuff", exist_ok=True)
    os.chdir('VaryingNumCollPointsRunStuff')
    ...
    lrnr.WriteResults(data_set_name=data_set, file_name=data_set+f"_{nCpnts}ColPoints", precision=5, true_eq=crctEQ, errors=[err, errs], act_func='Tanh()', **res_dict)
...
if __name__=='__main__':
    ...
    parser.add_argument('-num_col_pnts', type=int, required=True, help='Number of Collocation points to use throughout training. ')
    ...
    main(dflag=args.dset, ver=args.ver, data_slct_type=dtype, nCpnts=args.num_col_pnts, jobId=args.JobID, arrayId=args.arrayID, node=args.node)