...
from EqLearner import EqLearner1D, OldEqLearner1D
...
def main(dflag:int, ver:int, data_slct_type:str, jobId:int, arrayId:int, node:str):
    ...
    # (Dname, nDpnts, nTrn, nCpnts, noise, data_seed, sptl_ord, tmp_order, polyDeg, 
    #             numLyrsU, nPrLU, prd_stuff, four_stuff, rnd_wght_fct,
    #             ado_adms_epochs, ADO_iters, Kfolds, ADO_alphas, 
    #             trnEpchs, lrn_rt, btchSz, preAlpha, preGamma, xtra,
    #             pst_epchs, pst_alpha, pst_lrn_rt) = OptimizedEQsModelParameters(dFlag=dflag, ver=ver, run=jobVer, data_type=data_slct_type, computer=node)
    ...
    slvr = Cross_Val_RFE_V2(Kfolds=Kfolds, alpha=torch.zeros(size=(1,), device=dvc), normalize=True, annealing_factor=2, criteria="coefficient_value", selection="individual", best=False, ADO_iters=ADO_iters, thrshld_val=1e-4)
    # slvr = Cross_Val_RFE_V2(Kfolds=Kfolds, alpha=torch.zeros(size=(10,), device=dvc), normalize=True, annealing_factor=2, criteria="coefficient_value", selection="individual", best=False, ADO_iters=ADO_iters, thrshld_val=1e-4)
    ...
    saveName = 'OptimSettingsResultsForSlurmJob'+str(jobID)+'ArrayNum'+str(jobVer)
    sv_mdl_name = 'OptimSettingsLrnrForSlurmJob'+str(jobID)+'ArrayNum'+str(jobVer)
    ...
    if prd_stuff or four_stuff or rnd_wght_fct:
        nn = Networks.MLP(in_dim=2, num_layers=numLyrsU, hid_dim=nPrLU,out_dim=1,
    ...
    run_time = 0
    strt = perf_counter()
    # lrnr.AdamsOptimTraining(mode="pre", alpha=preAlpha, gamma=preGamma, min_epochs=100, max_epochs=200, 
    #                         lrn_rt=lrn_rt, lp_ord=1.0, outputFreq=100,
    #                         betas=(0.9, 0.999), threshold=False,)
    lrnr.Adams_Pretraining(alpha=1.0, gamma=1e-6, epochs=1000, lrn_rt=0.001, lp_ord=1.0, tstFreq=50, betas=(0.9,0.999))
    ...
    # pre_fig.savefig(fname=os.path.join(loss_plots_dir,f"PreTrainingLossesForSlurmJob{jobID})ArrayNum{jobVer}.png"), format='png')
    ...
    # pre_l1_fig.savefig(fname=os.path.join(loss_plots_dir,f"L1_LossesPreTrainedForSlurmJob{jobID})ArrayNum{jobVer}.png"), format='png')
    
    strt = perf_counter()
    # lrnr.ADO_Training(iters=2, train_alphas=ADO_alphas/ADO_alphas, LBFGS=False, lbfgs_epochs=10, Early_Term=True, Save_File_Name=None, Early_RFE_Term=True)
    lrnr.ADO_Training(iters=3, LBFGS=False, adms_epochs=800, lbfgs_epochs=10, train_alphas=(ADO_alphas/ADO_alphas)[:3], Early_Term=True, Save_File_Name=None,)
    run_time += (perf_counter() - strt)
    ...
    #     ado_axs["Data"].semilogy(lrnr._ADOtrnDataLs[ado_iter,:,0], 'b', label="Trn Data")
    #     ado_axs["Data"].semilogy(lrnr._ADOtstDataLs[ado_iter,:,0], 'rx', label="Tst Data")
    #     ado_axs["Data"].legend()
    #     ado_axs["Data"].set_title("Data Loss")

    #     ado_axs["EQ"].semilogy(lrnr._ADOtrnColloLs[ado_iter,:,0],'b', label="Trn Data")
    #     ado_axs["EQ"].semilogy(lrnr._ADOtstColloLs[ado_iter,:,0],'rx', label="Tst Data")
    #     ado_axs["EQ"].legend()
    #     ado_axs["EQ"].set_title("EQ Loss")

    #     ado_axs["Tot"].semilogy(lrnr._ADOTrnLosses[ado_iter,:,0], 'b-.', label="Trn Data")
    #     ado_axs["Tot"].semilogy(lrnr._ADOTstLosses[ado_iter,:,0], 'rx', label="Tst Data")
    #     ado_axs["Tot"].legend()
    #     ado_axs["Tot"].set_title(r"Combined loss = $\mathcal{L}_{D} + \alpha \mathcal{L}_{pde}$");
    
    #     ado_fig.savefig(fname=os.path.join(loss_plots_dir,f"AdoIter{ado_iter}EqLossesForSlurmJob{jobID})ArrayNum{jobVer}.png"), format='png')
    ...
    # pst_fig.savefig(fname=os.path.join(loss_plots_dir,f"PostTrainingLossesForSlurmJob{jobID})ArrayNum{jobVer}.png"), format='png')
    
    # pst_l1_fig, pst_l1_ax = plt.subplots(nrows=1, ncols=1, sharex=False, sharey=False, squeeze=True, layout="constrained")
    # pst_l1_ax.semilogy(lrnr.AdamsPstLpLoss, 'k.', label="Calced L1")
    # pst_l1_fig.savefig(fname=os.path.join(loss_plots_dir,f"L1_LossesPostTrainedForSlurmJob{jobID})ArrayNum{jobVer}.png"), format='png')
    ...
    lrnr.WriteResults(data_set_name=data_set, file_name=data_set+"DefaultVer3Settings", precision=5, true_eq=crctEQ, errors=[err, errs], act_func='Tanh()', **res_dict)
