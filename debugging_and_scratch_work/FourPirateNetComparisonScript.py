...
class KlnGrd_Net(torch.nn.Module):
    ...
    def __init__(self, 
        ...
        for i in range(n_hid_lyr):
            ...
            self.mduls[f"PiBtle{i}InLyr"] = torch.nn.Linear(in_features=embd_dim, out_features=nodes_per_lyr, bias=True)
            ...
            self.mduls[f"PiBtle{i}OutLyr"] = torch.nn.Linear(in_features=nodes_per_lyr, out_features=embd_dim, bias=True)
        ...
        self.mduls['OutLayer'] = torch.nn.Linear(in_features=embd_dim, out_features=out_dim, bias=True)
    ...
    def forward(self, x:torch.Tensor):
        ...
        x = torch.cat((torch.cos(torch.matmul(x, self.kernel)),torch.sin(torch.matmul(x, self.kernel)) ), dim=1)
...
def main(dflag:int, ver:int, data_slct_type:str, jobId:int, arrayId:int, node:str):
    ...
    saveName = 'FourPirateNetResultsForSlurmJob'+str(jobID)+'ArrayNum'+str(jobVer)
    ...
    os.makedirs(name="FourPirateNetRunStuff", exist_ok=True)
    os.chdir('FourPirateNetRunStuff')
    ...
    lrnr.WriteResults(data_set_name=data_set, file_name=data_set+"FourPirateNet", precision=5, true_eq=crctEQ, errors=[err, errs], act_func='Tanh()', **res_dict)
