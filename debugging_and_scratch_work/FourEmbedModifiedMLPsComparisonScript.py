...
class KlnGrd_Net(torch.nn.Module):
    ...
    def __init__(self, 
        ...
        self.mduls[f"U"] = torch.nn.Linear(in_features=embd_dim, out_features=nodes_per_lyr, bias=True)
        torch.nn.init.xavier_normal_(self.mduls[f"U"].weight.data, gain=1.41)
        self.mduls['U_Act_Func'] = torch.nn.Tanh()

        self.mduls[f"V"] = torch.nn.Linear(in_features=embd_dim, out_features=nodes_per_lyr, bias=True)
        torch.nn.init.xavier_normal_(self.mduls[f"V"].weight.data, gain=1.41)
        self.mduls['V_Act_Func'] = torch.nn.Tanh()

        self.mduls[f"HidnLay{0}"] = torch.nn.Linear(in_features=embd_dim, out_features=nodes_per_lyr, bias=True)
        torch.nn.init.xavier_normal_(self.mduls[f"HidnLay{0}"].weight.data, gain=1.41)
        self.mduls['ActFunc0'] = torch.nn.Tanh()

        for i in range(1, n_hid_lyr+1):
            self.mduls[f"HidnLay{i}"] = torch.nn.Linear(in_features=nodes_per_lyr, out_features=nodes_per_lyr, bias=True)
            torch.nn.init.xavier_normal_(self.mduls[f"HidnLay{i}"].weight.data, gain=1.41)
            self.mduls[f"ActFunc{i}"] = torch.nn.Tanh()

        self.mduls['OutLayer'] = torch.nn.Linear(in_features=nodes_per_lyr, out_features=out_dim, bias=True)
    ...
    def forward(self, x:torch.Tensor):
        ...
        U = self.mduls['U_Act_Func'](self.mduls[f"U"](y))
        V = self.mduls['V_Act_Func'](self.mduls[f"V"](y))
        for i in range(self.num_layers+1):
            y = self.mduls[f"HidnLay{i}"](y)
            y = self.mduls[f"ActFunc{i}"](y)
            y = y*U + (1-y)*V

        y = self.mduls['OutLayer'](y)
        return y*(x[:,0:1].pow(2) - 1)
...
class PeriodicBC_Network(torch.nn.Module):
    ...
    def forward(self, x:torch.Tensor):
        ...
        embedded = torch.cat((torch.cos(torch.matmul(embedded, self.kernel)),torch.sin(torch.matmul(embedded, self.kernel)) ), dim=1)
        U = self.mduls['U_Act_Func'](self.mduls[f"U"](embedded))
        V = self.mduls['V_Act_Func'](self.mduls[f"V"](embedded))
        for i in range(self.num_layers+1):
            embedded = self.mduls[f"HidnLay{i}"](embedded)
            embedded = self.mduls[f"ActFunc{i}"](embedded)
...
def main(dflag:int, ver:int, data_slct_type:str, jobId:int, arrayId:int, node:str):
    ...
    saveName = 'FourEmbededModMlpsResultsForSlurmJob'+str(jobID)+'ArrayNum'+str(jobVer)
    ...
    os.makedirs(name="FourEmbedModifiedMlpsRunStuff", exist_ok=True)
    os.chdir('FourEmbedModifiedMlpsRunStuff')
    ...
    lrnr.WriteResults(data_set_name=data_set, file_name=data_set+"FourEmbdModMlps", precision=5, true_eq=crctEQ, errors=[err, errs], act_func='Tanh()', **res_dict)
